...
int partial_pivot_serial(DATA_TYPE A[N][N], int n, int k) {
    int max_row = k;
    for (int i = k + 1; i < n; i++) {
    ...
    if (max_row != k) {
        // Scambia le righe k e max_row
        for (int j = 0; j < n; j++) {
            DATA_TYPE temp = A[k][j];
            A[k][j] = A[max_row][j];
            A[max_row][j] = temp;
        }
    }
...
void kernel_lu_serial(int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n)) {
     // Stampa la matrice dopo ogni passo
   printf("seriale\n");
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            printf("%.6f ", A[i][j]);
    ...
    for (int k = 0; k < n; k++) {
       if ( partial_pivot_serial(A, n, k) == 1)
	      continue; 
	 printf("seriale passo %d\n",k);
        for (int i = 0; i < n; i++) {
    	    for (int j = 0; j < n; j++) {
           	 printf("%.6f ", A[i][j]);
       	    }
